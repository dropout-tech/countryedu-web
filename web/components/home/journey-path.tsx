...
function TimelinePathDrawing({ lowPerf }: { lowPerf: boolean }) {
  ...
  return (
    ...
    >
      ...
      <motion.path
        d={TIMELINE_PATH_D}
        stroke={`url(#${boltId})`}
        strokeWidth={lowPerf ? 2.2 : 3}
        strokeLinecap="round"
        strokeLinejoin="round"
        fill="none"
        pathLength={1}
        strokeDasharray={lowPerf ? "0.09 0.91" : "0.11 0.89"}
        animate={{
          strokeDashoffset: [0, -1],
          opacity: lowPerf ? [0.2, 0.72, 0.2] : [0.2, 0.95, 0.2],
        }}
        transition={{
          duration: lowPerf ? 2.8 : 1.7,
          repeat: Infinity,
          ease: "linear",
        }}
        vectorEffect="non-scaling-stroke"
    ...
    >
      ...
      >
        <motion.div
          ...
          transition={{
            duration: lowPerf ? 0.35 : 2.5,
            repeat: lowPerf ? 0 : Infinity,
            ease: "easeInOut",
          }}
        />
...
export function JourneyPath() {
  ...
  const [isCoarseDevice, setIsCoarseDevice] = useState(false)

  useEffect(() => {
    if (typeof window === "undefined") return
    const mq = window.matchMedia("(max-width: 1024px), (pointer: coarse)")
    const update = () => setIsCoarseDevice(mq.matches)
    update()
    mq.addEventListener("change", update)
    return () => mq.removeEventListener("change", update)
  }, [])

  const lowPerf = prefersReducedMotion || isCoarseDevice
      ...
      <div className="pointer-events-none absolute inset-0 z-0">
        <motion.div
          ...
          style={{
            ...
            filter: lowPerf ? "blur(42px)" : "blur(72px)",
          }}
          animate={lowPerf ? undefined : { scale: [1, 1.12, 1], opacity: [0.45, 0.65, 0.45] }}
          transition={lowPerf ? undefined : { duration: 11, repeat: Infinity, ease: "easeInOut" }}
        />
        <motion.div
          ...
          style={{
            ...
            filter: lowPerf ? "blur(46px)" : "blur(80px)",
            opacity: lowPerf ? 0.26 : undefined,
          }}
          animate={lowPerf ? undefined : { scale: [1, 1.15, 1], opacity: [0.35, 0.5, 0.35] }}
          transition={lowPerf ? undefined : { duration: 13, repeat: Infinity, ease: "easeInOut", delay: 2 }}
        />
      </div>
      ...
      <div className="container relative z-10 mx-auto px-4">
        ...
        <div className="relative mx-auto max-w-4xl">
          <div className="pointer-events-none absolute inset-0 z-0 min-h-full">
            <TimelinePathDrawing lowPerf={lowPerf} />
          </div>

          <div className="relative z-10 space-y-16 py-8 md:space-y-24">
            {journeyNodes.map((node, index) => (
              <div key={node.id} className="relative">
                <JourneyNode node={node} index={index} lowPerf={lowPerf} />
              </div>
            ))}
          </div>
        </div>
